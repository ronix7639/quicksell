...
const User = () => {
  ...
  useEffect(() => {
    const groupedByUser = apiData.users.reduce((acc, user) => {
      acc[user.name] = apiData.tickets.filter(
        (task) => task.userId === user.id
      );
      return acc;
    }, {});
    setTasksByUser(groupedByUser);
  }, []);

  return (
    <div className="priority-container">
      {Object.keys(tasksByUser).map((user) => (
        <div className="priority-item" key={user}>
          <div className="priority-header">
            <div>
              <span style={{ fontWeight: "bold" }}>{user}</span>
              <span> {tasksByUser[user].length}</span>
            </div>
            <div className="icons">
